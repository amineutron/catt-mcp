...
# Racine du depot (le code vit dans catt_mcp/) : config.yaml a cote du serveur
...
class CattController:
    ...
    def _start_sync_watcher(self, video_id: str, offset: float = 3.3) -> str:
        ...
        try:
            ...
            # module du paquet (pas un chemin de fichier) : marche installe
            # (wheel, uvx) comme depuis le depot ; PYTHONPATH couvre le second cas
            racine = str(_REPO_ROOT)
            env = {**os.environ, "PYTHONPATH": os.pathsep.join(filter(None, [racine, os.environ.get("PYTHONPATH")]))}
            process = subprocess.Popen(
                [sys.executable, "-m", "catt_mcp.sync_watcher", self.tv_host, self.adb_path, video_id, str(offset)],
                env=env,
                stdout=log_file,
                stderr=log_file,
    ...
    def _stop_sync_watcher(self) -> str:
        ...
        ancien = _REPO_ROOT / ".sync_watcher.pid"   # emplacement d'avant
...
def cli(argv: list[str] | None = None) -> None:
    ...
    --help et --version repondent sans configuration ni appareil."""
    import argparse
    import asyncio as _asyncio

    from catt_mcp import __version__

    parser = argparse.ArgumentParser(
        prog="catt-mcp",
        description="MCP server to cast YouTube videos and media to a Chromecast or DLNA device (catt), stdio transport.",
        epilog="Configuration: CATT_DEVICE (default device name or IP), TV_HOST (Android TV IP for the ADB-based "
               "tools), CATT_CONFIG (path to a config.yaml). Dual-cast tools need Linux + Hyprland, playerctl and Firefox.",
    )
    parser.add_argument("--version", action="version", version=f"catt-mcp {__version__}")
