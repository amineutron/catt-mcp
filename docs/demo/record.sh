...
ROOT="$(cd "$HERE/../.." && pwd)"
cd "$ROOT"
export DEMO_SHOW='uv run python server.py   # depot catt-mcp, CATT_DEVICE dans l'environnement'
asciinema rec --overwrite --cols 100 --rows 30 --idle-time-limit 2 \
  --command "uv run python $HERE/mcp_demo.py 'uv run python server.py' cast_scan" /tmp/demo.cast
